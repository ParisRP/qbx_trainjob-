...
-- NOUVEL ÉVÉNEMENT : Pour quitter le métier de train
RegisterNetEvent('qbx_trainjob:server:exitTrainJob', function()
    local source = source
    ...
    if IsTrainDriver(source) then
        -- Définir le métier sur "unemployed" (chômeur)
        if Config.Framework == "QBCore" then
            player.Functions.SetJob("unemployed", 0)
        end
        
        -- Arrêter le job en cours si actif
        if activeJobs[source] then
            activeJobs[source] = nil
            TriggerClientEvent('QBCore:Notify', source, "Train job stopped and you are no longer a train driver.", "success")
        else
            TriggerClientEvent('QBCore:Notify', source, "You are no longer a train driver.", "success")
        end
        
        -- Log l'action
        Log("Player " .. GetPlayerName(source) .. " left the train job", "info")
    else
        TriggerClientEvent('QBCore:Notify', source, "You are not a train driver.", "error")
    end
end)
